...
func _ready() -> void:
    ...
    sfx_click = get_node_or_null("click") as AudioStreamPlayer  # make sure node name matches
...
func start_loading(path: String) -> void:
    ...
    # Play sound when loading appears
    if sfx_click:
        sfx_click.play()
...
func _change_scene() -> void:
    ...
    tree.set_current_scene(scene_instance)  # make the new scene current
    
    if current_scene != null and is_instance_valid(current_scene):
        current_scene.queue_free()
    
    # Play sound when loading disappears
    if sfx_click:
        sfx_click.play()
